...
/*First I group the total amount sponsored by sponsor_id and save the output into a new table "max_sponsor"*/
DROP TABLE IF EXISTS max_sponsor;
CREATE TABLE max_sponsor AS
SELECT sponsor_id, SUM(sponsed_amount) AS total_amount
FROM Video_sponsor_bridge
GROUP BY sponsor_id;

/*Then I join the "Sponsor" table with the newly created table "max_sponsor" on the "sponsor_id" column.
I did this based on a condition that selects the maximum total amount sponsored by a sponsor
which is calculated from the "max_sponsor" table. Based on this result, I then picked the required columns 
from the "Sponsor" table and the "max_sponsor" table.*/

SELECT Sponsor.first_name, Sponsor.last_name, Sponsor.phone, max_sponsor.total_amount
FROM Sponsor
JOIN max_sponsor 
ON Sponsor.sponsor_id = max_sponsor.sponsor_id 
WHERE max_sponsor.total_amount IN (
	SELECT MAX(max_sponsor.total_amount) FROM max_sponsor);